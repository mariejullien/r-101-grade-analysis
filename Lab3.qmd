...
grades |>
  group_by(Group) |>
  summarise(nb_missed_exam = sum(is.na(Exam))) |>
  ggplot(aes(x=Group, y=nb_missed_exam)) +geom_col() + labs(title="Nb of students  missed exam per group", y="Nb of students missed exam")
```
...
```{r}
ggplot(missinggrades, aes(x=totalnb_missing)) + geom_histogram(binwidth = 1, alpha=0.9, fill="blue", color="black") +
